...
public class TransactionController {
	...
	@GetMapping("/byId/{userId}/{pageNo}/{pageSize}")
	public ResponseEntity<?> allTransactionsByUserId(@PathVariable int userId, @PathVariable int pageNo, @PathVariable int pageSize){
        try {
            HashMap<String, Object> mapSuccess = new HashMap<>();
		    mapSuccess.put("status", "success");
		    mapSuccess.put("data", transactionService.findTransactionByUserId(pageNo, pageSize, userId));
		    mapSuccess.put("message", "List transaksi complete ");
